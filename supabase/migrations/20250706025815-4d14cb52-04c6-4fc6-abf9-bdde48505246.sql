...
-- Recreate admin master function with proper permissions
...
-- Remove max_storage_gb column from organizations table
ALTER TABLE organizations DROP COLUMN IF EXISTS max_storage_gb;

-- Update subscription plans enum to be more clear
DROP TYPE IF EXISTS subscription_plan_enum CASCADE;
CREATE TYPE subscription_plan_enum AS ENUM ('Free', 'Basic', 'Premium');

-- Add back subscription_plan column with new enum
ALTER TABLE organizations 
ADD COLUMN IF NOT EXISTS subscription_plan subscription_plan_enum NOT NULL DEFAULT 'Free';
